...
function provider(requests:Request[]){ return { async request(request:Request){ requests.push(request); if(request.method==='wallet_switchEthereumChain')return null; if(request.method==='eth_chainId')return `0x${studioDevnet.id.toString(16)}`; if(request.method==='eth_sendTransaction')return hash; throw new Error(`Unexpected ${request.method}`) } } }
